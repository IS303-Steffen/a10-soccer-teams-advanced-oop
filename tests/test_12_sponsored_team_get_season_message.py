...
from conftest import (
    ...
    unmangle_keys,
    unmangle_name,
    unmangle_and_collapse_keys,
    default_module_to_test
)

def test_12_sponsored_team_get_get_season_message(current_test_name, input_test_cases, class_test_cases):
    try:
        ...
        for class_result in class_results_list:
            ...
            for index, method_result in enumerate(method_results_list, start=1):
                ...
                initial_obj_state_vars_raw = method_result.get('initial_obj_state')[0].get('instance_variables')

                # First unmangle all keys; then collapse duplicates preferring the subclass ("SponsoredTeam")
                initial_obj_state_vars_unmangled = unmangle_keys(initial_obj_state_vars_raw)
                initial_obj_state_vars_collapsed = unmangle_and_collapse_keys(initial_obj_state_vars_unmangled, preferred_class="SponsoredTeam")

                initial_state_vars_str = '\n'.join(
                    f"{normalize_text(k)}: {normalize_text(v)}"
                    for k, v in initial_obj_state_vars_collapsed.items())
                    ...
                    for run_index, (expected_update_variable_name, expected_update_values) in enumerate(expected_object_updates_list.items()):
                        ...
                        for actual_var_name_raw, initial_var_value in initial_obj_state_vars_raw.items():
                            ...
                            if expected_name_match and expected_value_match:
                                ...
                                break
                        # I want the variables to be normalized, but also preserve the private variable naming
