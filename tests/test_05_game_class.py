...
def test_05_game_class(current_test_name, input_test_cases, class_test_cases):
    try:
        ...
        for class_result in class_results_list:
            ...
            for expected_var_name, expected_var_value in class_result['init_expected_values'].items():
                ...
                for actual_var_name, actual_var_value in class_result['actual_object'].get("__data__").items():
                    ...
                    # check the variable names (use logical names)
                    ...
                    # if this field holds a nested object dict, compare its __data__ after unmangling all keys
                    if normalized_actual_name in ['hometeam', 'awayteam'] and isinstance(actual_var_value, dict):
                        # unwrap the serialized object
                        actual_var_value = actual_var_value.get('__data__')
                        # unmangle all keys inside the nested object so __wins, __losses are logical
                        actual_var_value = unmangle_keys(actual_var_value)

                    # if the expected side is a nested object dict (as in your class_test_cases), unmangle it too
                    if normalized_expected_name in ['hometeam', 'awayteam'] and isinstance(expected_var_value, dict):
                        expected_var_value = unmangle_keys(expected_var_value)

                    # now compare values (after normalization)
                ...
                if not expected_value_found:
                    ...
                    types = {'home_team': 'SoccerTeam or SponsoredTeam', 'away_team': 'SoccerTeam or SponsoredTeam'} # if the object should have other custom objects inside it, list the variable names and their class name here.
                    ...
                    # show students a LOGICAL (unmangled) view of the whole object
                    actual_data_unmangled = unmangle_keys(class_result['actual_object'].get("__data__"))
                    actual_data_unmangled.pop('methods', None)

                    actual_obj_values_normalized = prettify_dictionary(
                        normalize_text(actual_data_unmangled)
                    )
