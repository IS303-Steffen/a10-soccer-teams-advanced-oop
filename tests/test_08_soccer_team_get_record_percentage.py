...
def test_08_soccer_team_get_record_percentage(current_test_name, input_test_cases, class_test_cases):
    try:
        ...
        method_to_test = 'get_record_percentage'
                ...
                if expected_object_updates_list:
                    for run_index, (expected_update_variable_name, expected_update_values) in enumerate(expected_object_updates_list.items()):
                        ...
                        if initial_value_found:
                            ...
                            normalized_student_var_name_str = normalized_student_var_name.replace('wins', '__wins').replace('losses', '__losses')
                            if not normalized_actual_final_value == normalized_expected_final_value:
                                formatted = format_error_message(
                                    custom_message=(f"The method `{method_to_test}` is provided with the following argument(s) other than \"self\":\n"
                                                    ...
                                                    f"```\n{normalized_expected_name_str}: {normalized_expected_final_value} (data type: {type(normalized_expected_final_value).__name__})\n```\n"
                                                    f"In your {class_name} object, this was your initial value:\n"
                                                    f"### Your initial value:\n"
                                                    f"```\n{normalized_student_var_name_str}: {normalized_actual_initial_var_value} (data type: {type(normalized_actual_initial_var_value).__name__})\n```\n"
                                                    f"But the final value of {normalized_student_var_name} after calling the method `{method_to_test}` was:\n"
                                                    f"### Your final value:\n"
                                                    f"```\n{normalized_student_var_name_str}: {normalized_actual_final_value} (data type: {type(normalized_actual_final_value).__name__})\n```\n"
